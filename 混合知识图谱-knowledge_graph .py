...
class DualLayerKnowledgeGraph:
    ...
    def __init__(self, case_data: Dict[str, Any], model_name: str):
        ...
        self.client = OpenAI(
            api_key="sk-qlbkpfwrmmjaqhlubivnqobvynqittrojgkfyqmbssjaxdkh",
            base_url="https://cloud.siliconflow.cn/models?target=Qwen/Qwen3-30B-A3B-Thinking-2507"
        )
    ...
    def retrieve_by_entities(self, user_input: str, entities: List[str], top_k: int = 5) -> Dict[str, List[Dict]]:
            ...
            for ent in entities:
                ...
                for hit in hits:
                    triple_str = f"{hit['data'].get('subject')} {hit['data'].get('predicate')} {hit['data'].get('object')} {hit['data'].get('narrative', '')}"
            ...
            # 2. 调用 LLM 仅做“相关性筛选”（不要求排序）
            candidates_formatted = "\n".join([f"{c['id']}. {c['description']}" for c in candidate_pool])
            
            system_prompt = (
                "你是一个审讯系统专家。你的任务是从候选列表中挑选出所有与【用户提问】内容相关的知识三元组 ID。\n"
                "返回所有相关的 ID 列表。\n"
                "格式要求：只返回 JSON 数组，例如：[1, 5, 2]。"
            )
            ...
            try:
                ...
                if content.startswith("["):
                    ...
                    relevant_ids = json.loads(content).get("selected_ids", [])

              
                selected_candidates = [candidate_pool[idx] for idx in relevant_ids if 0 <= idx < len(candidate_pool)]

                def calculate_relevance(cand):
                    sim_score = difflib.SequenceMatcher(None, user_input, cand["description"]).ratio()
                    

                    strength = cand["original_data"].get("evidence_strength", 0.5)
                    
                    return sim_score * 0.7 + strength * 0.3 # 综合评分

                # 执行降序排序
                selected_candidates.sort(key=calculate_relevance, reverse=True)
